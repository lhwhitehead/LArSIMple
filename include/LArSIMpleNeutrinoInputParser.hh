...
inline LArSIMpleNeutrinoInteractionType LArSIMpleNeutrinoInputParser::ConvertGiBUUCode(int code, bool iscc)
{
  ...
  // QE
  if(code == 1 && iscc)  return LArSIMpleNeutrinoInteractionType::kCCQE;
  if(code == 1 && !iscc) return LArSIMpleNeutrinoInteractionType::kNCQE;
  // Res
  if(code > 1 && code < 34 && iscc)  return LArSIMpleNeutrinoInteractionType::kCCRES;
  if(code > 1 && code < 34 && !iscc) return LArSIMpleNeutrinoInteractionType::kNCRES;
  // DIS
  if(code == 34 && iscc)  return LArSIMpleNeutrinoInteractionType::kCCDIS;
  if(code == 34 && !iscc) return LArSIMpleNeutrinoInteractionType::kNCDIS;
  // Other
  if(code > 34 && code < 38 && iscc)  return LArSIMpleNeutrinoInteractionType::kCCOther;
  if(code > 34 && code < 38 && !iscc) return LArSIMpleNeutrinoInteractionType::kNCOther;
...
inline int LArSIMpleNeutrinoInputParser::GetPDGCodeFromGiBUU(const int pid, const int charge)
{
  ...
  // Nucleons
  if (pid == 1 && charge == 1) return 2212; // proton
  if (pid == 1 && charge == 0) return 2112; // neutron

  // Charged leptons
  if (pid == 901 && charge == -1) return 11; // electron  
  if (pid == 901 && charge == 1) return -11; // positron  
  if (pid == 902 && charge == -1) return 13; // muon  
  if (pid == 902 && charge == 1) return -13; // anti muon  
  if (pid == 903 && charge == -1) return 15; // tau
  if (pid == 903 && charge == 1) return -15; // anti tau  

  // Neutrinos (neutrino or antineutrino doesn't matter for now)
  if (pid == 911) return 12; // electron neutrino
  if (pid == 912) return 14; // electron neutrino
  if (pid == 913) return 16; // electron neutrino
 
  // Pions
  if (pid == 101 && charge == 1)  return 211;  // pi plus
  if (pid == 101 && charge == 0)  return 111;  // pi zero
  if (pid == 101 && charge == -1) return -211; // pi minus

  // Kaons
  if (pid == 110 && charge == 1)  return 321;  // K plus
  if (pid == 110 && charge == 0)  return 311;  // K zero
  if (pid == 111 && charge == -1) return -321; // K minus
  if (pid == 111 && charge == 0)  return -311; // K zero bar

  // Deltas
  if (pid == 2 && charge == 2) return 2224;  // delta plus plus
  if (pid == 2 && charge == 1) return 2214;  // delta plus
  if (pid == 2 && charge == 0) return 2114;  // delta zero
  if (pid == 2 && charge == -1) return 1114; // delta minus

  // Lambda
  if (pid == 32) return 3122; // Lambda zero 

  // Sigmas
  if (pid == 33 && charge == 1) return 3222;  // Sigma plus
  if (pid == 33 && charge == 0) return 3212;  // Sigma zero
  if (pid == 33 && charge == -1) return 3112; // Sigma minus

  // Other mesons
  if (pid == 102) return 221; // eta
  if (pid == 103 && charge == 1)  return 213;  // rho plus
  if (pid == 103 && charge == 0)  return 113;  // rho zero
  if (pid == 103 && charge == -1) return -213; // rho minus
  if (pid == 105) return 223; // omega
